...
├── requirements.txt
│
├── step1_explore.py            # 1. Dataset exploration and statistics
├── step2_clean.py              # 2. Corrupt-image and MD5 duplicate removal
├── step3_augment_split.py      # 3. Stratified 70/15/15 split and augmentation
├── step3_1_rice_fix.py         # 3.1 Class-structure fix for the Rice Leaf dataset
│
├── phase2_detection.py         # YOLOv8m disease-region detection
├── phase2_segmentation.py      # U-Net lesion segmentation
├── phase2_classification.py    # Classification training
│
├── priority2_baselines.py      # VGG16, MobileNetV2, EfficientNetB0, ResNet50 baselines
├── priority3_convnext.py       # AgriConvNet: ConvNeXt-Small + 4×CBAM (proposed model)
├── priority4_blockchain.py     # SHA-256 ledger, smart contract, blockchain benchmark
├── priority5_ablation.py       # CBAM ablation study
└── gradcam_visualization.py    # Grad-CAM maps and attention concentration
```
...
|---|---|---|
| `step1_explore.py` | Sec 3.1–3.2 | Table 2, Fig 1–2 |
| `step2_clean.py` | Sec 3.3, Algorithm 1 | Cleaned datasets (1,080 images removed) |
| `step3_augment_split.py`, `step3_1_rice_fix.py` | Sec 3.4, Algorithm 1 | Table 3 |
| `phase2_detection.py` | Sec 4.2, 5.3 | Table 8 (detection) |
| `phase2_segmentation.py` | Sec 4.3, 5.3 | Table 8 (segmentation) |
| `phase2_classification.py` | Sec 4.4 | Classification training |
| `priority2_baselines.py` | Sec 5.2 | Table 6 and 7 (baselines) |
| `priority3_convnext.py` | Sec 4.4, 5.2, 5.4 | Table 4, 6, 9 (AgriConvNet) |
| `priority4_blockchain.py` | Sec 4.5, 5.5 | Table 10 |
| `priority5_ablation.py` | Sec 6 | Table 12 |
| `gradcam_visualization.py` | Sec 5.6 | Table 11, Fig 11–12 |
...
```bash
git clone https://github.com/ranjithkumaranandan/AgriChainNet.git
cd AgriChainNet
pip install -r requirements.txt
```

Install the CUDA build of PyTorch that matches your GPU from [pytorch.org](https://pytorch.org). The experiments used PyTorch 2.11 with CUDA 12.8.

## Usage

Download the four datasets (see [Datasets](#datasets)) into the project root, then run the scripts in order:

```bash
# 1. Data preparation
python step1_explore.py
python step2_clean.py
python step3_augment_split.py
python step3_1_rice_fix.py

# 2. Detection and segmentation
python phase2_detection.py
python phase2_segmentation.py
python phase2_classification.py

# 3. Classification: baselines and proposed AgriConvNet
python priority2_baselines.py
python priority3_convnext.py

# 4. Blockchain layer
python priority4_blockchain.py

# 5. Ablation and explainability
python priority5_ablation.py
python gradcam_visualization.py
```

> **Note:** Dataset and output paths are set at the top of each script. Edit them to match your local folder layout before running.
...
Trained model weights are available from the author on reasonable request.
